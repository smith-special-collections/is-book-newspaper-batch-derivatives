...
#unbatchprocess.process(FILE_LIST_FILENAME, 'convert "$objFileName" "$objDirName/JP2.jp2"', concurrentProcesses=39)
...
#runbatchprocess.process(FILE_LIST_FILENAME, 'tesseract "$objFileName" "$objDirName/tesseract-output" HOCR TXT && mv "$objDirName/tesseract-output.hocr" "$objDirName/HOCR" && mv "$objDirName/tesseract-output.txt" "$objDirName/OCR"', concurrentProcesses=10)

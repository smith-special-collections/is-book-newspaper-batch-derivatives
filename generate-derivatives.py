...
# Generating TECHMD.xml
logging.info('Generating TECHMD.xml files with Fits')
runbatchprocess.process(FILE_LIST_FILENAME, 'fits.sh -i "$objFileName" -o "$objDirName/TECHMD.xml" &> "$objDirName/.fits-`date +%s`.log"', concurrentProcesses=20)
